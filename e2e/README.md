...
Testes de ponta a ponta com [Playwright](https://playwright.dev), cobrindo o fluxo completo pela UI: cadastro do negócio (onboarding self-service, incluindo CPF/CNPJ e escolha de plano — usados depois para cobrar a assinatura) → login → criação de serviço → cadastro de profissional vinculado ao serviço → geração de horários → agendamento público (escolha de serviço, profissional e horário) → confirmação.
